...
- **_Is a non-linear data structure since each element points to one or more elements of the same type._**
- This is given an element, there is no single path to follow. The element that points to another is called the parent, while the pointed element is known as the child. - - All elements have a parent except the root. It can be said that a tree is made up of subtrees, thus highlighting its recursive nature._**
...
**_A binary search tree (ABB), is a binary tree in which for every element, the elements larger than it are located in its right branch, while the smaller elements go in its left branch. Each element is stored only once so there are no repeated elements._**
...
**_Un árbol binario de búsqueda (ABB), es un árbol binario en el cual para todo elemento, los elementos mayores a él, se ubican en su rama derecha, mientras que los elementos menores van en su rama izquierda. Cada elemento se almacena una sola vez por lo que no existen elementos repetidos._**
...
**_Los punteros tienen que ser del tipo árbol, ya que apuntarán a un nodo del mismo tipo, este seria un ejemplo de como se seria el tipo arbol ABB._**
